...
def process_queue_user(queue_user_usecase, admin_auth, user_auth):
    ...
    @queue_user.get(
        "/",
        defaults={"queue_user_id": None},
        endpoint="get_by_admin_without_queue_user_id",
    )
    @queue_user.get("/<int:queue_user_id>", endpoint="get_by_admin_with_queue_user_id")
    @admin_auth.login_required
    @swag_from(
        "../docs/queue_user/get_queue_user_using_auth_admin.yaml",
        endpoint="queue_user.get_by_admin_without_queue_user_id",
    )
    @swag_from(
        "../docs/queue_user/get_queue_user_by_id_using_auth_admin.yaml",
        endpoint="queue_user.get_by_admin_with_queue_user_id",
    )
    def get_queue_user_by_auth_admin(queue_user_id):

        result = queue_user_usecase.get_queue_user_by_admin(
    ...
    @queue_user.get(
        "/status_queue_user/",
        defaults={"queue_user_id": None},
        endpoint="get_by_user_without_queue_user_id",
    )
    @queue_user.get(
        "/status_queue_user/<int:queue_user_id>",
        endpoint="get_by_user_with_queue_user_id",
    )
    @user_auth.login_required
    @swag_from(
        "../docs/queue_user/get_queue_user_by_id_using_auth_user.yaml",
        endpoint="queue_user.get_by_user_with_queue_user_id",
    )
    @swag_from(
        "../docs/queue_user/get_queue_user_using_auth_user.yaml",
        endpoint="queue_user.get_by_user_without_queue_user_id",
    )
    def get_queue_user_by_auth_user(queue_user_id):

        result = queue_user_usecase.get_queue_user_by_user(
            user_auth.current_user(), queue_user_id
        )
