...
from src.lib.model import Admin, User, Queue, QueueUser, db
...
@swag_from(
    "../docs/user/get_user_using_auth_admin.yaml",
    endpoint="user.without_id",
    methods=["GET"],
)
@swag_from(
    "../docs/user/get_user_by_id_using_auth_admin.yaml",
    endpoint="user.with_id",
    methods=["GET"],
)
@swag_from(
    "../docs/user/post_user_using_auth_admin.yaml",
    endpoint="user.without_id",
    methods=["POST"],
)
def post_and_get_user_by_auth_admin(id):
    ...
    if request.method == "GET":
        ...
        queue_user_result = QueueUser.query.filter(
            QueueUser.queue_id.in_([queue.id for queue in queue_result])
        ).all()
        filters = (
