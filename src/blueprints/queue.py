...
from src.lib.model import Queue, Admin, QueueUser, User, db
...
@swag_from(
    ...
    endpoint="queue.without_id",
    methods=["GET"],
)
@swag_from(
    "../docs/queue/get_queue_by_id_using_auth_admin.yaml",
    endpoint="queue.with_id",
    methods=["GET"],
)
@swag_from(
    "../docs/queue/post_queue_using_auth_admin.yaml",
    endpoint="queue.without_id",
    methods=["POST"],
)
...
def get_queue_by_auth_user():
    ...
    queue_result = Queue.query.filter(
        Queue.id.in_([queue_user.queue_id for queue_user in queue_user_result])
    ).all()
