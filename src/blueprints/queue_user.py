...
@queue_user.route(
    "/", defaults={"id": None}, methods=["POST", "GET"], endpoint="without_id"
)
...
@swag_from(
    ...
    endpoint="queue_user.without_id",
    methods=["GET"],
)
@swag_from(
    "../docs/queue_user/get_queue_user_by_id_using_auth_admin.yaml",
    endpoint="queue_user.with_id",
    methods=["GET"],
)
@swag_from(
    "../docs/queue_user/post_queue_user_using_auth_admin.yaml",
    endpoint="queue_user.without_id",
    methods=["POST"],
)
def post_and_get_queue_user_by_auth_admin(id):
    ...
    if request.method == "GET":
        ...
        if not queue_result or not user_result:
            return jsonify({"message": "queue or user not found!"}), HTTP_404_NOT_FOUND
...
def post_queue_user_by_auth_user(id):
    ...
    queue_user_result = QueueUser.query.filter_by(
        queue_id=id, user_id=user_result.id
    ).first()

    if not user_result or not queue_result:
        return jsonify({"message": "queue or user not found!"}), HTTP_404_NOT_FOUND
    elif queue_user_result:
        return jsonify({"message": "user already join this queue!"}), HTTP_302_FOUND

    queue_user = QueueUser(queue_id=id, user_id=user_result.id)
    ...
    return jsonify({"queue_id": id, "user_id": user_result.id}), HTTP_201_CREATED
...
def delete_queue_user(id):
    admin_result = Admin.query.filter_by(email=auth_admin.current_user()).first()
    queue_result = Queue.query.filter_by(admin_id=admin_result.id).all()
    queue_user_result = QueueUser.query.filter(
        QueueUser.queue_id.in_([queue.id for queue in queue_result]), QueueUser.id == id
    ).first()
